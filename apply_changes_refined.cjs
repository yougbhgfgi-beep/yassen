...
// Replacement 1: Birthday Cake Text (Already done, but for safety)
...
const loginRegex = /<div className="relative inline-block p-4 bg-white\/5 rounded-full border border-white\/10 mb-4">[\s\S]*?<Heart className="w-12 h-12 text-\[#8b5cf6\] animate-pulse" \/>[\s\S]*?<\/div>/;
...
if (loginRegex.test(content)) {
    content = content.replace(loginRegex, newLoginImage);
    console.log("Replaced login icon.");
} else {
    console.log("Could not find login icon regex.");
}

// Replacement 3: Hero Background
const heroRegex = /<svg viewBox="0 0 100 100" className="absolute inset-4 w-\[calc\(100%-32px\)\] h-\[calc\(100%-32px\)\] text-black\/40 backdrop-blur-md drop-shadow-2xl transition-transform duration-700 group-hover:scale-105" fill="currentColor">[\s\S]*?<path d="M50 88\.932l-5\.6-5\.1C23\.6 64\.932 10 52\.632 10 38\.432c0-11\.6 8\.8-20\.4 20\.4-20\.4 6\.7 0 13\.1 3\.2 17\.2 8\.1 4\.1-4\.9 10\.5-8\.1 17\.2-8\.1 11.6 0 20.4 8.8 20.4 20.4 0 14.2-13.6 26.5-34.4 45.4l-5.6 5.1z" \/>[\s\S]*?<\/svg>/;
...
if (heroRegex.test(content)) {
    content = content.replace(heroRegex, newHeroBg);
    console.log("Replaced hero background.");
} else {
    console.log("Could not find hero background regex.");
}
